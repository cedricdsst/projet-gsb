...
$sql4 = "SELECT DISTINCT motif FROM rapport";
...
<main>
    ...
    <form action="rapport_post.php" method="post">
       ...
       <div class="line">
        <legend>Motif de visite</legend> 
        <select class="inputs"name="motif" >
            <?php  
            while ($data4 = $reponse4->fetch()) {
                echo "<option value='{$data4['motif']}'>{$data4['motif']}</option>";
                 }
            ?>
        </select>
        </div>
        ...
        <br>
        <div class="line">
        <input type="submit" class="btn" name="report" value="Envoyer le rapport">
        </div>
